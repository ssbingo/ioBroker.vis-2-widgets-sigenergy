...
Monitorización y control del cargador AC Sigenergy (EVAC). Muestra la potencia de carga, el estado del sistema, la potencia nominal, la corriente nominal y el consumo total de energía. Las alarmas se resaltan con colores. La insignia de estado muestra el estado de carga simplificado según IEC 61851-1 (Inicialización, Libre, Conectado, Cargando, Error); al pasar el ratón por encima se muestra una explicación detallada del estado actual. La corriente de carga se puede ajustar directamente mediante un control deslizante (de 6 A hasta la corriente nominal del cargador); el límite superior se puede restringir adicionalmente con el ajuste del widget `sig_maxCurrent`. Durante una carga activa, el botón Start está bloqueado y el botón Stop se resalta.
...
* (ssbingo) El conjunto de widgets ahora informa la versión correcta en la consola del navegador

### 1.8.5 (2026-09-07)
* (ssbingo) Cargador AC: el control deslizante de corriente de carga ahora está limitado a la corriente nominal del cargador; nuevo ajuste del widget `sig_maxCurrent` para un límite superior manual (evita errores Modbus al ajustar más que la corriente nominal)
* (ssbingo) Cargador AC: el estado del sistema cubre ahora todos los estados IEC 61851-1 (0–7: inicialización, libre, conectado, cargando, error); al pasar el ratón sobre la insignia de estado se muestra una explicación detallada

### 1.8.4 (2026-09-04)
* (ssbingo) Requisito mínimo de admin reducido a >=7.8.23 (admin 8 ya no es necesario)
* (ssbingo) CI: ioBroker/testing-action-deploy fijado a la versión mayor v1; corregido el workflow de auto-merge de Dependabot
* (ssbingo) Dependencias actualizadas: @tsconfig/node22 22.0.6, @alcalzone/release-script-plugin-license 5.2.2
