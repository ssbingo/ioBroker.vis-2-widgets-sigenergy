![Логотип](../../admin/vis-2-widgets-sigenergy.png)
# ioBroker.vis-2-widgets-sigenergy

[![Версия NPM](https://img.shields.io/npm/v/iobroker.vis-2-widgets-sigenergy.svg)](https://www.npmjs.com/package/iobroker.vis-2-widgets-sigenergy)
[![Загрузки](https://img.shields.io/npm/dm/iobroker.vis-2-widgets-sigenergy.svg)](https://www.npmjs.com/package/iobroker.vis-2-widgets-sigenergy)
![Количество установок](https://iobroker.live/badges/vis-2-widgets-sigenergy-installed.svg)
![Текущая версия в стабильном репозитории](https://iobroker.live/badges/vis-2-widgets-sigenergy-stable.svg)

[![NPM](https://nodei.co/npm/iobroker.vis-2-widgets-sigenergy.png?downloads=true)](https://nodei.co/npm/iobroker.vis-2-widgets-sigenergy/)

**Тесты:** ![Тест и релиз](https://github.com/ssbingo/ioBroker.vis-2-widgets-sigenergy/workflows/Test%20and%20Release/badge.svg)

## Адаптер vis-2-widgets-sigenergy для ioBroker

Набор виджетов VIS-2 для адаптера накопителя энергии Sigenergy (`ioBroker.sigenergy`).
Содержит 8 виджетов для визуализации и управления потоком энергии, состоянием батареи, мощностью в реальном времени, дневной статистикой, зарядным устройством AC, зарядным устройством DC, инвертором и обзором микроинверторов SigenMicro. для визуализации и управления потоком энергии, состоянием аккумулятора, мощностью в реальном времени, дневной статистикой, AC-зарядным устройством, DC-зарядным устройством и инвертором.

## Требования

- ioBroker с установленным и настроенным адаптером `sigenergy`
- Адаптер ioBroker VIS-2 (≥ 2.0.0)

## Виджеты

### Диаграмма потока энергии
Отображает текущий поток энергии между солнечными панелями, аккумулятором, сетью и домом в виде анимированной SVG-диаграммы. Анимированные стрелки визуализируют активные соединения в реальном времени.

**OID:** `pvPower`, `essPower`, `gridActivePower`, `housePower`, `essSoc`

![Диаграмма потока энергии](../../img/widget-energiefluss.png)

#### Направления потока

| Точка данных | Значение > 0 | Значение < 0 |
|---|---|---|
| `essPower` | Аккумулятор заряжается → стрелка от центра к аккумулятору | Аккумулятор разряжается → стрелка от аккумулятора к центру |
| `gridActivePower` | Потребление из сети → стрелка от сети к центру | Отдача в сеть → стрелка от центра к сети |
| `pvPower` | PV вырабатывает → стрелка от PV к центру | — |
| `housePower` | Дом потребляет → стрелка от центра к дому | — |

### Состояние аккумулятора и прогнозы
Отображает SOC, SOH, мощность зарядки, а также прогнозы времени до полной зарядки, оставшегося времени работы, собственного потребления и уровня автономии.

**OID:** `essSoc`, `essSoh`, `essPower`, `batteryTimeToFull`, `batteryTimeRemaining`, `selfConsumptionRate`, `autarkyRate`

![Состояние аккумулятора](../../img/widget-batterie.png)

### Мощность в реальном времени
Компактный список всех текущих значений мощности с цветовой индикацией направления.

**OID:** `pvPower`, `essPower`, `gridActivePower`, `housePower`, `essSoc`

![Мощность в реальном времени](../../img/widget-leistung.png)

### Статистика энергии
Дневной обзор с уровнем автономии, собственным потреблением, изменением SOC, энергией заряда/разряда и покрытием аккумулятора.

**OID:** `autarkyRate`, `selfConsumptionRate`, `dayMaxSoc`, `dayMinSoc`, `essDailyChargeEnergy`, `essDailyDischargeEnergy`, `batteryCoverageToday`, `batteryDailyChargeTime`

![Статистика энергии](../../img/widget-statistiken.png)

### AC-зарядное устройство (Sigen EVAC)
Мониторинг и управление AC-зарядным устройством Sigenergy (EVAC). Отображает мощность зарядки, состояние системы, номинальную мощность, номинальный ток и общее потребление энергии. Аварийные сигналы выделяются цветом. Значок состояния показывает упрощённое состояние зарядки по IEC 61851-1 (Инициализация, Свободно, Подключено, Зарядка, Ошибка); при наведении курсора отображается подробное объяснение текущего состояния. Ток зарядки можно регулировать с помощью ползунка (от 6 А до номинального тока зарядного устройства); верхний предел можно дополнительно ограничить настройкой виджета `sig_maxCurrent`. Во время активной зарядки кнопка Start заблокирована, а кнопка Stop выделена.

**OID:** `acCharger.systemState`, `acCharger.chargingPower`, `acCharger.totalEnergyConsumed`, `acCharger.ratedPower`, `acCharger.ratedCurrent`, `acCharger.alarm1/2/3`, `acCharger.control.startStop`, `acCharger.control.outputCurrent`

![AC-зарядное устройство](../../img/widget-ac-charger.png)

### DC-зарядное устройство
Мониторинг и управление DC-зарядным устройством Sigenergy. Отображает выходную мощность, SOC автомобиля с индикатором прогресса, напряжение аккумулятора автомобиля, ток зарядки, а также энергию и продолжительность текущей сессии зарядки. Значок состояния показывает рабочее состояние зарядной станции (`dcCharger.runningState`: свободно, подключено/подготовка, запланировано, зарядка, разрядка, завершено, предупреждение, ошибка/недоступно); при наведении курсора отображается подробное объяснение. Во время активной зарядки или разрядки кнопка Start заблокирована, а кнопка Stop выделена. Если OID состояния не задан, он выводится из OID выходной мощности; без значения состояния значок определяется по выходной мощности, а подсказка объясняет причину в зависимости от версии протокола, определённой адаптером.

**OID:** `dcCharger.runningState`, `dcCharger.outputPower`, `dcCharger.vehicleSoc`, `dcCharger.vehicleBatteryVoltage`, `dcCharger.chargingCurrent`, `dcCharger.currentChargingCapacity`, `dcCharger.currentChargingDuration`, `dcCharger.control.startStop`, `info.protocolVersion` (`oid_protocol`)

![DC-зарядное устройство](../../img/widget-dc-charger.png)

### Инвертор
Комплексный мониторинг и управление инвертором с навигацией по вкладкам. Отображает рабочее состояние, данные мощности, температуры аккумулятора, фазные напряжения, все 5 регистров аварийных сигналов и информацию об устройстве (модель, серийный номер, прошивка).

| Вкладка | Содержание |
|---|---|
| **Мощность** | Активная мощность, мощность PV, мощность заряда/разряда аккумулятора, ползунок доли мощности (от −100 % до +100 %) |
| **Аккумулятор** | SOC и SOH с индикаторами, средняя температура/напряжение ячеек, макс./мин. температура |
| **Сеть** | Фазные напряжения L1/L2/L3, частота сети, коэффициент мощности, внутренняя температура PCS |
| **Аварии** | 5 регистров аварийных сигналов (PCS ×2, ESS, шлюз, DC-зарядное) с HEX-кодом и цветовой маркировкой |
| **Инфо** | Тип модели, серийный номер, версия прошивки, переключатель Remote-EMS |

![Инвертор](../../img/widget-inverter.png)

**OID:** `inverter.activePower`, `inverter.pvPower`, `inverter.essChargeDischargePower`, `inverter.runningState`, `inverter.essBatterySoc/Soh`, `inverter.essAvgCellTemperature/Voltage`, `inverter.phaseA/B/CVoltage`, `inverter.gridFrequency`, `inverter.pcsInternalTemp`, `inverter.alarm1–5`, `inverter.firmwareVersion`, `inverter.modelType`, `inverter.serialNumber`, `inverter.control.startStop`, `inverter.control.remoteEmsDispatchEnable`, `inverter.control.activePowerPercent`

### PV Power
Отображение до 3 PV-строк с актуальными значениями мощности и анимированными стрелками потока к гибридному инвертору. Цвета стрелок изменяются динамически в зависимости от мощности (оранжевый <1 кВт, жёлтый <2 кВт, зелёный >2 кВт).

#### Настройки виджета
| Параметр | Тип | По умолчанию | Описание |
|---|---|---|---|
| oid_pv1 … oid_pv3 | OID | sigenergy.0.plant.pv1Power … pv3Power | OID мощности PV-строк |
| oid_pvtotal | OID | sigenergy.0.plant.pvPower | OID общей мощности PV |
| sig_title | текст | PV Power | Заголовок виджета |
| sig_name1 … sig_name3 | текст | String 1 … String 3 | Настраиваемые имена строк |
| sig_darkmode | флажок | true | Тёмный / светлый режим |

![PV Power](../../img/PV-PowerOverview.png)

**OIDs:** `plant.pv1Power`, `plant.pv2Power`, `plant.pv3Power`, `plant.pvPower`

### Обзор SigenMicro
Обзор и детальное представление всех микроинверторов SigenMicro на шине Modbus. Вкладка 1 показывает все устройства в виде анимированного сетевого сегмента (топология Ethernet-шины с вертикальными ответвлениями). Каждая последующая вкладка отображает все 15 регистров соответствующего устройства в возрастающем порядке.

#### Динамический макет
| Устройства | Строки | Размер изображения |
|---|---|---|
| 1–5 | 1 строка | 80 × 90 пкс |
| 6–10 | 1 строка | 52 × 60 пкс |
| 11–15 | 2 строки | 46 × 52 пкс |
| 16–20 | 2 строки | 40 × 46 пкс |

### Уровень заряда автомобиля (EV SOC)
Отображает настраиваемое изображение автомобиля (например, Fiat 500e) в качестве центрального визуального элемента. Цветной значок в правом верхнем углу показывает символ молнии, текущий уровень заряда в процентах и надпись «LADESTAND». Прогресс-бар внизу виджета отражает текущий SOC. При активном состоянии зарядки значок пульсирует зелёным свечением.

#### Цветовая логика
| Уровень заряда | Цвет |
|---|---|
| ≤ 15 % | Красный (#f87171) |
| ≤ 35 % | Жёлтый (#fbbf24) |
| > 35 % | Зелёный (#4ade80) |

![Fahrzeug-Ladestand Widget](../../img/widget-autoLadestand.png)

**OIDs:** `oid_ev_soc`, `oid_charging`

## Оформление

Все виджеты поддерживают **светлый и тёмный режим**, переключаемый через настройку виджета `Тёмный режим`.

## Changelog
### 1.8.11 (2026-09-12)
* (ssbingo) CI: тесты пакета теперь выполняются на Node.js 22, 24 и 26 (новое задание `adapter-tests`, repository checker #41); `@iobroker/testing` обновлён с 5.3.0 до 6.2.1

### 1.8.10 (2026-09-07)
* (ssbingo) Запись `admin` удалена из `globalDependencies`: набор виджетов не имеет интерфейса администратора, поэтому требовать версию admin не нужно (repository checker S1091)

### 1.8.9 (2026-09-07)
* (ssbingo) DC-зарядное устройство: станция, помечающая регистр рабочего состояния как недействительный, больше не отображается с красным значком «Unbekannt». Протокол Sigenergy обозначает «регистр недействителен» установкой всех битов, и SigenStor EC **с** DC-зарядным устройством отвечает так для регистра 31513, тогда как соседние регистры (номинальная мощность, выработка PV, счётчики) читаются нормально. В этом случае значок определяется по выходной мощности, а подсказка поясняет, что это не проблема адаптера или конфигурации
* (ssbingo) DC-зарядное устройство: распознаётся и сырое значение 65535, поэтому значок корректен и с версиями адаптера до 3.3.1, которые передают это значение вместо отсутствия значения
* (ssbingo) DC-зарядное устройство: новая настройка OID `oid_protocol` (по умолчанию `sigenergy.0.info.protocolVersion`). Ранее версия протокола выводилась только из префикса экземпляра; такой OID VIS не подписывает, `vis.states` оставался пустым, и подсказка утверждала «версия протокола ещё не определена», хотя адаптер определил V2.9 при запуске. Объявленная как обычный атрибут `/id`, она подписывается как любой другой OID
* (ssbingo) DC-зарядное устройство: если версию протокола нельзя прочитать, подсказка больше не утверждает, что она не определена, а сообщает, что здесь она недоступна для чтения, и указывает на новую настройку
* (ssbingo) DC-зарядное устройство: переформулирована подсказка, показываемая, когда адаптер не сообщает состояние, хотя устройство объявляет протокол V2.8 или новее; она больше не утверждает, что требуется обновление адаптера, поскольку сама станция может пометить регистр как недействительный

### 1.8.8 (2026-09-07)
* (ssbingo) DC-зарядное устройство: если OID состояния не задан, он выводится из OID выходной мощности (…dcCharger.outputPower → …dcCharger.runningState), поэтому виджеты, размещённые до 1.8.7, показывают рабочее состояние без редактирования
* (ssbingo) DC-зарядное устройство: если рабочее состояние недоступно, подсказка объясняет причину в зависимости от версии протокола, определённой адаптером (`info.protocolVersion` / `info.protocolLevel`): регистр 31513 требует протокол Sigenergy V2.8; начиная с V2.8 она отсылает к журналу адаптера или обновлению адаптера; отрицательная выходная мощность отображается как разрядка

### 1.8.7 (2026-09-07)
* (ssbingo) DC-зарядное устройство: новый OID состояния `dcCharger.runningState` (по умолчанию) – значок состояния теперь показывает рабочее состояние зарядной станции (свободно, подключено/подготовка, запланировано, зарядка, разрядка, завершено, предупреждение, ошибка/недоступно) с подробным объяснением в подсказке; без OID значок, как и раньше, определяется по выходной мощности
* (ssbingo) DC-зарядное устройство: во время активной зарядки или разрядки кнопка Start заблокирована, а кнопка Stop выделена; отрицательная выходная мощность (разрядка) отображается фиолетовым

### 1.8.6 (2026-09-07)
* (ssbingo) AC-зарядное устройство: подсказка на значке состояния теперь отображается как отдельное всплывающее окно с фиксированным размером шрифта и непрозрачным фоном, поэтому она читаема независимо от размера виджета и больше не перекрывается содержимым виджета
* (ssbingo) AC-зарядное устройство: во время активной зарядки кнопка Start заблокирована, а кнопка Stop выделена
* (ssbingo) Набор виджетов теперь сообщает правильную версию в консоли браузера

### 1.8.5 (2026-09-07)
* (ssbingo) AC-зарядное устройство: ползунок тока зарядки теперь ограничен номинальным током зарядного устройства; новая настройка виджета `sig_maxCurrent` для ручного верхнего предела (предотвращает ошибки Modbus при установке значений выше номинального тока)
* (ssbingo) AC-зарядное устройство: состояние системы теперь охватывает все состояния IEC 61851-1 (0–7: инициализация, свободно, подключено, зарядка, ошибка); при наведении на значок состояния отображается подробное объяснение

### 1.8.4 (2026-09-04)
* (ssbingo) Минимальное требование к admin снижено до >=7.8.23 (admin 8 больше не требуется)
* (ssbingo) CI: ioBroker/testing-action-deploy закреплён на мажорной версии v1; исправлен workflow автослияния Dependabot
* (ssbingo) Обновлены зависимости: @tsconfig/node22 22.0.6, @alcalzone/release-script-plugin-license 5.2.2

### 1.8.3 (2026-08-05)
* (ssbingo) Объявлены минимальные требования: js-controller >=6.0.11, admin >=8.0.0, Node.js >=22
* (ssbingo) Обновлены зависимости: actions/checkout 7.0.1, ioBroker/testing-action-deploy 1.5.2, @iobroker/testing 5.3.0

### 1.8.2 (2026-06-28)
* (ssbingo) Обновлены CI-действия: actions/checkout до v7.0.0, ioBroker/testing-action-deploy до v1.5.0

### 1.8.1 (2026-06-08)
* (ssbingo) Исправлена синтаксическая ошибка JSON в io-package.json; в документацию добавлен снимок экрана виджета

### 1.8.0 (2026-06-08)
* (ssbingo) Новый виджет: «Уровень заряда автомобиля» — показывает настраиваемое изображение электромобиля с анимированной полосой SOC, цветовой индикацией уровня заряда (красный/жёлтый/зелёный) и опциональным мигающим значком зарядки

### 1.7.9 (2026-05-27)
* (ssbingo) Удалены устаревшие .eslintrc.json и .prettierignore

### 1.7.8 (2026-05-27)
* (ssbingo) Добавлено ESLint-линтование, CI обновлён до Node.js 24; адаптер требует node.js >= 22

### 1.7.7 (2026-04-20)
* (ssbingo) Текст больше не искажается при неравномерном масштабировании — буквы сохраняют пропорции, а контейнеры продолжают заполнять область виджета

### 1.7.6 (2026-04-20)
* (ssbingo) Масштабирование теперь неоднородное: ширина и высота реагируют независимо на изменения контейнера, обе оси остаются настраиваемыми отдельно

### 1.7.5 (2026-04-20)
* (ssbingo) Масштабирование виджета теперь реагирует и на изменение высоты — содержимое масштабируется пропорционально по обеим осям и центрируется в виджете

### 1.7.4 (2026-04-20)
* (ssbingo) Все 9 виджетов теперь масштабируют содержимое адаптивно к размеру виджета (шрифты, отступы, SVG, изображения)

### 1.7.3 (2026-04-20)
* (ssbingo) Все 9 виджетов теперь имеют единый фон на основе дизайна виджета PV-Power

## Лицензия
MIT License

Copyright (c) 2026 ssbingo <s.sternitzke@online.de>

Данная лицензия разрешает лицам, получившим копию данного программного обеспечения
и сопутствующей документации, безвозмездно использовать программное обеспечение
без ограничений, включая неограниченное право на использование, копирование,
изменение, слияние, публикацию, распространение, сублицензирование и/или продажу
копий программного обеспечения, а также лицам, которым предоставляется данное
программное обеспечение, при соблюдении следующих условий:

Указанное выше уведомление об авторском праве и данное разрешение должны быть
включены во все копии или значительные части данного программного обеспечения.

ДАННОЕ ПРОГРАММНОЕ ОБЕСПЕЧЕНИЕ ПРЕДОСТАВЛЯЕТСЯ «КАК ЕСТЬ», БЕЗ КАКИХ-ЛИБО ГАРАНТИЙ,
ЯВНЫХ ИЛИ ПОДРАЗУМЕВАЕМЫХ, ВКЛЮЧАЯ, НО НЕ ОГРАНИЧИВАЯСЬ ГАРАНТИЯМИ ТОВАРНОЙ ПРИГОДНОСТИ,
СООТВЕТСТВИЯ ПО ЕГО КОНКРЕТНОМУ НАЗНАЧЕНИЮ И НЕНАРУШЕНИЯ ПРАВ.

## Документация

- 🇷🇺 [Русский](../../doc/ru/README.md) — этот файл
- 🇩🇪 [Deutsch](../../doc/de/README.md)
- 🇬🇧 [English](../../README.md)
- 🇳🇱 [Nederlands](../../doc/nl/README.md)
- 🇫🇷 [Français](../../doc/fr/README.md)
- 🇮🇹 [Italiano](../../doc/it/README.md)
- 🇪🇸 [Español](../../doc/es/README.md)
- 🇵🇱 [Polski](../../doc/pl/README.md)
- 🇵🇹 [Português](../../doc/pt/README.md)
